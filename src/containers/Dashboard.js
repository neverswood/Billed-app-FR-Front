...
export default class {
  ...
  handleShowTickets(e, bills, index) {
    ...
    if(this.billsEventHandlerFlag === undefined){
      this.billsEventHandlerFlag = true
      $(`.bills-feed`).on('click', e => {
        if(e.target.matches(".bill-card,.bill-card>*")){
          const billElement = e.target.closest('.bill-card')
          const billElementId = billElement.id
          const billId = billElementId.split('open-bill')[1]
